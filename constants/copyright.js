export const mainContent = {
    title: "Яркие осенние \n тиражи",
    menu: [{title: "Главная"}, {title: "О лотерее"}, {title: "Победители"}, {title: "Как участвовать"}, {title: "Безопасность"}, {title: "Приложение"}],
    list: [{
        order: "тираж 1456",
        title: "Разыграем призы по 500 000₽",
        subtitle: "Победителей будет больше!",
        date: "Трансляция – 4 сентября",
    }, {
        order: "тираж 1457", title: "Разыграем загородные дома", date: "Трансляция –  11 сентября",
    }, {
        order: "тираж 1458",
        title: "Разыграем призы по 1 000 000 ₽",
        subtitle: "Победителей будет больше!",
        date: "Трансляция – 18 сентября",
    }],
    images: [{
        type: "1", image: "images/main/2.png",
    }, {
        type: "2", image: "images/main/3.png",
    }, {
        type: "3", image: "images/main/4.png",
    },],
    imageList: [
        {
            name: "bg",
            image: "images/main/bg.png"
        },
        {
            name: "bg2",
            image: "images/main/bg2.png"
        },
        {
            name: "logo",
            image: "images/main/logo.png"
        },
        {
            name: "home",
            image: "images/main/home.png"
        },
        {
            name: "home2",
            image: "images/main/home2.png"
        },
        {
            name: "prize",
            image: "images/main/prize.png"
        },
    ],
    chanel: {
        image: "images/main/chanel.png", text: "Смотрите трансляции тиражей \n в 8:20 на НТВ"
    },
    button: {
        text: "Купить билет",
    }
}


export const winnersContent = {
    list: [{
        image: "images/winners/1.png", name: "Людмила Новик", prize: "706 577 ₽"
    }, {
        image: "images/winners/2.png", name: "Людмила Новик", prize: "706 577 ₽"
    }, {
        image: "images/winners/3.png", name: "Людмила Новик", prize: "706 577 ₽"
    }, {
        image: "images/winners/1.png", name: "Людмила Новик", prize: "706 577 ₽"
    }, {
        image: "images/winners/2.png", name: "Людмила Новик", prize: "706 577 ₽"
    }, {
        image: "images/winners/3.png", name: "Людмила Новик", prize: "706 577 ₽"
    }, {
        image: "images/winners/1.png", name: "Людмила Новик", prize: "706 577 ₽"
    }, {
        image: "images/winners/2.png", name: "Людмила Новик", prize: "706 577 ₽"
    }, {
        image: "images/winners/3.png", name: "Людмила Новик", prize: "706 577 ₽"
    }],
    iconNav: "images/winners/next.svg",
    title: "Наши победители",
    sign: "Ещё больше историй смотрите в соцсетях",
    icons: ["images/winners/icon1.png", "images/winners/icon2.png", "images/winners/icon3.png", "images/winners/icon4.png",],

}


export const rulesContent = {
    title: "Как участвовать",
    list: [
        {
            num: "1",
            title: "Выберите \n билет",
            text: "Комбинации в билетах проставлены заранее.\n Вам просто нужно выбрать один \n или несколько билетов",
            image: "images/rules/1.png",
        },
        {
            num: "2",
            title: "Выберите билет",
            text: "Оплатить билет вы можете любым удобным для вас способом: · Банковские карты — Mastercard,   Visa, МИР; · Кошелёк «Столото»; · QIWI Кошелек; Кошелёк ЮMoney; · · Система быстрых платежей; · SberPay; · Альфа-Клик.",
            image: "images/rules/2.png",
        },
        {
            num: "3",
            title: "Выберите билет",
            text: "Комбинации в билетах проставлены заранее.\n Вам просто нужно выбрать один \n или несколько билетов",
            image: "images/rules/3.png",
        },
        {
            num: "4",
            title: "Выберите билет",
            text: "Комбинации в билетах проставлены заранее.\n Вам просто нужно выбрать один или несколько билетов",
            image: "images/rules/4.png",
        },
    ],
    iconNav: "images/winners/next.svg",
    button: {
        text: "Участвовать",
    },
    mButton: {
        text: "Хочу так же!",
    }
}


export const infoContent = {
    title: "Покупать билеты\nонлайн – безопасно",
    list: [
        {
            mod: "active",
            title: "Безопасная регистрация",
            text: "При регистрации вы указываете только свой электронный адрес и номер телефона. На этот номер мы будем отправлять вам смс с паролем для получения выигрышей на сайте.",
            image: "images/info/1.png",
        },
        {
            title: "Безопасность данных",
            text: "Мы не сохраняем номера банковских карт, платёжные пароли и другие данные, которые вы вводите при оплате билетов. Вся информация хранится только на стороне платёжной системы, которую вы выбираете сами.",
            image: "images/info/2.png"
        },
        {
            title: "Государственные лотереи",
            text: "Организаторами всех лотерей в России являются Министерство спорта и Министерство финансов.",
            image: "images/info/3.png",
        },
        {
            title: "Ваш билет никогда не потеряется",
            text: "Вся информация о купленных билетах хранится в вашем личном кабинете. Если возникнут проблемы, вы сможете обратиться в нашу службу поддержки. Мы всегда вам поможем.",
            image: "images/info/4.png",
        }
    ],
    link: {
        text: "Смотреть \nвидео",
        icon: "images/info/icon.svg",
    }
}

export const downloadListContent = {
    title: "Скачайте мобильное\nприложение «Столото»",
    list: [
        {
            icon: "images/downloadList/1.svg",
            text: "Удобная и безопасная оплата"
        },
        {
            icon: "images/downloadList/2.svg",
            text: "Быстрый вывод денег на карту"
        },
        {
            icon: "images/downloadList/3.svg",
            text: "Лотереи всегда под рукой"
        },
        {
            icon: "images/downloadList/4.svg",
            text: "Push-уведомления о выигрышах"
        }
    ],
    loadButton: {
        text: "Скачать",
        icons: [
            "images/downloadList/5.svg",
            "images/downloadList/6.svg",
            "images/downloadList/7.svg",
            "images/downloadList/8.svg",
        ]
    },
    button: {
        text: "Повысить шансы"
    },
    image: "images/downloadList/phone.png",
}

export const footerContent = {
    logo: "images/footer/logo.png",
    text: "«Джекпот 800 миллионов» указывает на минимальный гарантированный размер средств призового фонда, разыгрываемого в категории «Джекпот» в тиражах № 1456 (розыгрыш 04.09.2022), 1457 (розыгрыш 11.09.2022), 1458 (розыгрыш 18.09.2022). «ВГЛ 4 Спорт» (алгоритм определения выигрышей № 3), коммерческое наименование — «Русское лото», срок проведения — до 31.12.2029. Информацию об организаторе лотереи, о правилах ее проведения, о призовом фонде лотереи, количестве призов или выигрышей, о сроках, месте и порядке получения призов или выигрышей можно узнать на сайте www.stoloto.ru или по тел. 8 900 555-00-55.Розыгрыши проводятся еженедельно. Выигрыши носят вероятностный характер. Лотерея не является способом заработка и источником дохода. АО «ТК «Центр», ОГРН 1127746385095, адрес: 109316, Москва, Волгоградский пр-т, д. 43, корп. 3, этаж 10, пом. XXV, ком. 13Б. Реклама. 18+",
    icons: [
        "images/footer/1.svg",
        "images/footer/2.svg",
        "images/footer/3.svg",
        "images/footer/4.svg",
    ],
    list: [
        {
            mod: "emblem",
            images: [
                {image: "images/footer/5.png"},
                {image: "images/footer/6.png"},
            ]
        },
        {
            mod: "white",
            text: "Организаторами лотерей являются Министерство спорта РФ \nи Министерство финансов РФ."
        },
        {
            mod: "partners",
            images: [
                {image: "images/footer/7.png"},
                {image: "images/footer/8.png"},
            ]
        },
        {
            mod: "white",
            text: "АО «ТК «Центр» («Столото») состоит во Всемирной и Европейской \nлотерейных ассоциациях (The World Lottery Association и The European \nLotteries Association), придерживаясь высоких стандартов по \nвопросам соблюдения прав участников, случайности определения \nпобедителей, своевременной выплаты выигрышей, \nконфиденциальности личных данных. "
        },
        {
            text: "WLA — международное объединение компаний-операторов \nгосударственных лотерей."
        },
        {
            text: "ELA — объединение лотерейных компанийиз Европы и соседних \nрегионов."
        }
    ]
}

export const bgContent = {
    decoration: ["images/bg/4.png", "images/bg/6.png",],
    bgs: [
        {
            name: "bg",
            image: "images/bg/sphera.png",
        },
        {
            name: "bg2",
            image: "images/bg/bg.png",

        },
        {
            name: "bg3",
            image: "images/bg/bg3.png",
        },
        {
            name: "m-bg",
            image: "images/bg/mSphera.png",
        },
        {
            name: "bg-md",
            image: "images/bg/bg-md.png",
        }
    ]
}


export const prizesContent = {
    title: "Уютные осенние призы ждут вас!",
    bg: "images/bg/bg4.png",
    button: {
        text: "Купить"
    },
}